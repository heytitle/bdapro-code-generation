 ...
 *
 */
public final class MySorter<T> implements InMemorySorter<T> {

	private static final Logger LOG = LoggerFactory.getLogger(MySorter.class);
	...
	public MySorter(TypeSerializer<T> serializer, TypeComparator<T> comparator, List<MemorySegment> memory) {
		this(serializer, comparator, memory, DEFAULT_MAX_NORMALIZED_KEY_LEN);
	}

	public MySorter(TypeSerializer<T> serializer, TypeComparator<T> comparator,
					List<MemorySegment> memory, int maxNormalizedKeyBytes)
	{
		...
		Log.debug("ShiftBits " + this.shiftBitsIndexEntriesPerSegment);
